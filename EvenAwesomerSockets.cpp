...
WSADATA winsockdata; //Windows socket information
...
int NetworkManager::Initialize(){
	...
	{
		std::cout << "Failed to initialize Winsock. Press any key to exit." << std::endl;
		std::cin.ignore().get();
		return 420;
	}

	ZeroMemory(&addrinfo, sizeof(addrinfo)); //Fill addrinfo
	addrinfo.ai_family = AF_UNSPEC; // Return addresses from any family (IPV4/6)
...
int NetworkManager::Connect(std::string sIP, bool IpIsDomainName, std::string sPort)
{
	...
	ioctlsocket(ConnectSocket, FIONBIO, &iMode); //Make the connection socket non-blocking (Asynchronous).
	return 0;
}
...
void NetworkManager::Disconnect()
{
	...
	closesocket(NetworkManager::GetSocket()); //Dispose of the socket
	WSACleanup(); //Clean the memory
	return;
}